...
for ii=1:length(APbinID)
    TempLocation=find([Everything(cc).Embryo(ee).SpotDiff(pp).APBin]==APbinID(ii));%ismembertol([Everything.APBin],APbinID(ii))%find(abs([Everything.APBin]-APbinID(ii))<0.001);
    TwoSpotsVect=[]; OneSpotVect=[]; NoSpotVect=[];
    for jj=1:length(TempLocation)
        if ~isempty(Everything(cc).Embryo(zz).SpotDiff(jj).TwoFract)
        TwoSpotsVect(jj)=Everything(cc).Embryo(zz).SpotDiff(jj).TwoFract;
        OneSpotVect(jj)=Everything(cc).Embryo(zz).SpotDiff(jj).OneFract;
        NoSpotVect(jj)=Everything(cc).Embryo(zz).SpotDiff(jj).NoFract;
        end
    end
    AvgTwoSpots(cc,ii)=nanmean(TwoSpotsVect);
    AvgTwoSpotsSD(cc,ii)=std(TwoSpotsVect);
    AvgTwoSpotsSE(cc,ii)=AvgTwoSpotsSD(cc,ii)/sqrt(length(TwoSpotsVect));
    AvgOneSpot(cc,ii)=nanmean(OneSpotVect);
    AvgNoSpot(cc,ii)=nanmean(NoSpotVect);
end
