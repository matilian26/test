%% plot mean correlation at each AP position for each embryo of a construct 
...
%figure
%h(jj)=plot(APBins,MeanAPCorr,'LineWidth',2)
%         hold on
%         xlabel('Mean AP bin')
%         ylabel('Avg correlation of spots at AP position')
%         title(['2 spot correlation',ConstructList(ii)]);
%         ylim([0 1]);
...
%     end
    ...
    clearvars -except ConstructList DropboxFolder ConCorrMean AllCorr  APBins MeanAPCorr MeanAllCorr
end
...
legend('Kr Both', 'Kr Dist', 'Kr Prox','Kr BothSep','Kr 2xDist');
